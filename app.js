...
// 360° panorama: local assets (e.g. Insta 360) or online environments
const PANORAMA_PATHS = [ 'assets/panorama.jpg', 'assets/panorama.png', 'assets/360.jpg', 'assets/360.png' ];

// Real environments online – seated café/restaurant first (person sitting, cutout on table), then others
// Poly Haven CC0: Comfy Café, Bush Restaurant (tonemapped JPG = TextureLoader)
...
function addTableWindow() {
  ...
  const legMat = new THREE.MeshLambertMaterial({ color: 0x3d2817 });
  ...
  tablePlane = new THREE.Mesh(cutoutGeo, videoMat);
  tablePlane.rotation.x = -Math.PI / 2;
  tablePlane.position.set(0, tableY + 0.008, tableZ - 0.002);
  tableGroup.add(tablePlane);

  const edgeMat = new THREE.MeshBasicMaterial({ color: 0x2c1810 });
  const edgeH = 0.012, edgeInset = 0.006;
  [
    ...
    edge.position.set(x, tableY + 0.01, z);
    tableGroup.add(edge);
  });
...
function applyOrientation() {
  ...
  const pitch = THREE.MathUtils.degToRad(-currentBeta) + SEATED_LOOK_DOWN_RAD;
  camera.rotation.order = 'YXZ';
